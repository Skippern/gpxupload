...
def __get_from_overpass(search_string, last):
    ...
    try:
        ...
        if result is not None:
            ...
            if len(elements) < 1:
                __LOG.debug("__get_from_overpass: json in get_data contains empty ['elements']!")
            return result
        __LOG.error("__get_from_overpass: API returned None")

    except overpass.errors.OverpassSyntaxError as e:
        __LOG.fatal("__get_from_overpass: OverpassSyntaxError: %s" % e.message)
        sys.exit(125)
    except overpass.errors.ServerRuntimeError as e:
        __LOG.error("__get_from_overpass: ServerRuntimeError: %s" % e.message)
    except overpass.errors.UnknownOverpassError as e:
        __LOG.error("__get_from_overpass: UnknownOverpassError: %s" % e.message)
    except overpass.errors.TimeoutError as e:
        __LOG.error("__get_from_overpass: TimeoutError: %s" % e.message)
    except overpass.errors.MultipleRequestsError as e:
        __LOG.error("__get_from_overpass: MultipleRequestsError: %s" % e.message)
        delay = __OVERPASS_OVERLOAD_DELAY
    except overpass.errors.ServerLoadError as e:
        __LOG.error("__get_from_overpass: ServerLoadError: %s" % e.message)
        delay = __OVERPASS_OVERLOAD_DELAY
    except requests.exceptions.ConnectionError as e:
        __LOG.error("__get_from_overpass: ConnectionError: %s" % e.message)
    except TypeError as e:
        __LOG.fatal("__get_from_overpass: json.TypeError: %s" % e.message)
        sys.exit(124)
    except ValueError as e:
        __LOG.fatal("__get_from_overpass: No Valid JSON Loaded (json.ValueError): %s" % e.message)
        sys.exit(123)
    except KeyError as e:
        __LOG.fatal("__get_from_overpass: json in get_data does not contain ['elements']: %s" % e.message)
        sys.exit(122)

    if not last:
        __LOG.debug("__get_from_overpass: Waiting for retry in %s seconds" % delay)
        time.sleep(delay)
    return None
...
def get_relations_in_bbox(min_lat, min_long, max_lat, max_long, level=2):
    ...
    """
    if min_lat == max_lat:
        __LOG.error("get_relations_in_bbox: Latitude %s equal %s - EXPANDING", str(min_lat), str(max_lat))
        min_lat = min_lat - 0.0000001
        max_lat = max_lat + 0.0000001
    if min_long == max_long:
        __LOG.error("get_relations_in_bbox: Longitude %s equal %s - EXPANDING", str(min_long), str(max_long))
        min_long = min_long - 0.0000001
        max_long = max_long + 0.0000001
    if min_lat > max_lat:
        __LOG.error("get_relations_in_bbox: Latitude %s greater than %s - SWAPPING", str(min_lat), str(max_lat))
        min_lat, max_lat = gpx_utils.swap(min_lat, max_lat)
    if min_long > max_long:
        __LOG.error("get_relations_in_bbox: Longitude %s greater than %s - SWAPPING", str(min_long), str(max_long))
